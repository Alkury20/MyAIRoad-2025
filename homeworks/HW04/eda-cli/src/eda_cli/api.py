...
@app.post(
    ...
    tags=["quality"],
    summary="Вывод всех флагов по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> FlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, проверяет по всем флагам
    и возвращает их значение.
    """
    ...
    flags_serializable: dict[str, Any] = {}
    for key, value in flags.items():
        if hasattr(value, 'item'):  # numpy scalar
            flags_serializable[key] = value.item()
        elif isinstance(value, (bool, int, float, str)) or value is None:
            flags_serializable[key] = value
        else:
            flags_serializable[key] = str(value)  # fallback для других типов
