...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    has_many_skipping_values = int(missing_df["missing_count"].values.sum())
    flags["has_many_skipping_values"] = has_many_skipping_values
    max_missing_share = float(missing_df["missing_share"].max()) if not missing_df.empty else 0.0
    flags["max_missing_share"] = max_missing_share
    flags["too_many_missing"] = max_missing_share > 0.5

    flags["has_constant_columns"] = any(col.unique == 1 for col in summary.columns if col.non_null > 0)
    ...
    for col in summary.columns:
        if not col.is_numeric and col.non_null > 0:
            ...
            if cardinality_ratio > high_cardinality_threshold and col.unique > 10:
    ...
    for col in summary.columns:
        ...
        if col_name_lower.endswith("_id") or col_name_lower == "id":
            # Если колонка выглядит как ID, проверяем уникальность
    ...
    score -= has_many_skipping_values * 0.01
